...
EXPECTED_PROFIT_PER_TRADE = (
    0.003  # be realistic. Setting this too high will eliminate potentially good solutions
)
EXPECTED_AVE_PROFIT = (
    0.001  # used to assess actual profit vs desired profit. Typical is 0.4% (0.004)
)
EXPECTED_MONTHLY_PROFIT = (
    0.15  # used to assess actual profit vs desired profit. Typical is 15% (0.15)
)
    ...
    def hyperopt_loss_function(
        ...
        **kwargs,
        ...
        if config["exchange"]["name"]:
            if (config["exchange"]["name"] == "kucoin") or (
                config["exchange"]["name"] == "ascendex"
            ):
        ...
        if backtest_stats["profit_total"]:
            ...
            abs_profit_loss = profit_sum / (
                config["max_open_trades"] * config["stake_amount"] * num_months
            )
        else:
        ...
        if backtest_stats["starting_balance"]:
            expected_sum = backtest_stats["starting_balance"] * (
                1.0 + EXPECTED_MONTHLY_PROFIT * num_months
            )
        else:
        ...
        if weight_win_loss_ratio > 0.0:
            if winning_count < (1.0 * losing_count):
                if debug_level > 1:
                    print(
                        " \tWinning count below goal: {:.0f} vs {:.0f}".format(
        ...
        if down_stdev != 0:
            sortino_ratio_loss = (
                -1.0 * (expected_returns_mean / down_stdev * np.sqrt(365)) / 10000.0
            )
        else:
            if debug_level > 1:
        ...
        if ((result < 0.0) and (debug_level > 0)) or (debug_level > 1):
            print(
                " \tPabs:{:.2f} Pave:{:.2f} n:{:.2f} dur:{:.2f} w/l:{:.2f} "
